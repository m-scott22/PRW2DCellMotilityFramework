function [Tj,Y,P,fulllogACF,P_lower,P_upper,lnACF,ACF]=velocitycutting2D(Alltraj,Ncells,T,alphaP,mse)
%Choose robustness
...
for cell=1:Ncells
    ...
    for i=1:length(T)
        ACF_cell(cell,i)= Alltraj(cell,1,1).*Alltraj(cell,i,1)+ Alltraj(cell,1,2).*Alltraj(cell,i,2);
    end
end
...
for j=1:length(cutoffs)-1
    ...
    if length(Tj)>5
        %Fit a line of best fit to the log data but change sign
